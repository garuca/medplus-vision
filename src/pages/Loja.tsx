...
import { Search, Filter, X, Minus, Plus, ArrowLeft, ArrowRight, Package, TrendingUp, Award } from "lucide-react";
...
const categoriasLoja = [
  ...
  { slug: "instrumentais-cirurgicos", nome: "Instrumentais Cirúrgicos", cor: "from-violet-500 to-violet-600" },
...
export default function Loja() {
  ...
  // Usa hook para alternar entre local e supabase
  const { produtos: allProducts, loading, source } = useProdutos();

  // Função para gerar link de produto mantendo source
  const getProdutoLink = (id: string) => {
    const params = new URLSearchParams(window.location.search);
    const urlSource = params.get("source");
    if (urlSource === "supabase") {
      return `/produto/${id}?source=supabase`;
    }
    return `/produto/${id}`;
  };
  ...
  const produtosPaginados = produtosFiltrados.slice((pagina - 1) * produtosPorPagina, pagina * produtosPorPagina);
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-6 relative overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-600 via-cyan-500 to-blue-600 px-8 text-white">
            ...
            <div className="relative flex h-40 items-center">
              <div>
                ...
                <p className="text-cyan-100">Equipamentos médicos de qualidade para profissionais e instituições</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-muted-foreground">{produtosFiltrados.length} produtos</span>
              <button onClick={() => setMostrarFiltros(!mostrarFiltros)} className="btn-glass rounded-xl px-4 py-2 text-sm flex items-center gap-2">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {["Urgência e Emergência", "Sinais Vitais", "Diagnóstico", "Instrumentais Cirúrgicos"].map((cat) => (
            ...
            <div className={`${mostrarFiltros ? "lg:col-span-3" : "lg:col-span-4"}`}>
              ...
              {totalPaginas > 1 && (
                <div className="mt-8 flex justify-center gap-2">
                  <button onClick={() => setPagina(Math.max(1, pagina - 1))} disabled={pagina === 1} className="btn-glass rounded-xl px-4 py-2 text-sm disabled:opacity-50">
                    <ArrowLeft className="h-4 w-4" />
                  </button>
                  {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((p) => (
                    <button key={p} onClick={() => setPagina(p)} className={`btn-glass rounded-xl px-4 py-2 text-sm ${pagina === p ? "bg-primary text-white" : ""}`}>
                      {p}
                    </button>
                  ))}
                  <button onClick={() => setPagina(Math.min(totalPaginas, pagina + 1))} disabled={pagina === totalPaginas} className="btn-glass rounded-xl px-4 py-2 text-sm disabled:opacity-50">
