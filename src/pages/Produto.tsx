...
import { ArrowLeft, Minus, Plus, Heart, Share2, Check, MessageCircle, ShoppingCart, Truck, CreditCard, Lock, Loader2 } from "lucide-react";
...
export default function Produto() {
  ...
  const { produto, loading, error } = useProdutoById(params?.id || "");
    ...
    <>
      <section className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <Link to="/loja" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-6">
...
<div className="grid gap-6 lg:grid-cols-2 lg:gap-8">
            ...
            <div className="relative flex flex-col gap-3">
              <div className="aspect-[4/3] overflow-hidden rounded-2xl bg-white/80 shadow-xl shadow-blue-500/5">
                <img src={imagemAtual} alt={produto.nome} className="h-full w-full object-contain p-4" />
              </div>
              {/* Miniaturas */}
              {todasImagens.length > 1 && (
                <div className="flex gap-2 justify-center">
                  {todasImagens.map((img, i) => (
                    ...
                    >
                      <img src={img} alt={`Vista ${i + 1}`} className="h-full w-full object-contain rounded-md" />
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="flex flex-col">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{produto.marca}</span>
              <h1 className="mt-1 text-xl font-bold text-foreground leading-tight">{produto.nome}</h1>
              ...
              <div className="mt-4">
                <p className="text-sm text-muted-foreground">
                  em até <span className="font-semibold text-foreground">{Math.ceil((produto.precoPromocional || produto.preco) / 2)}x</span> de R$ {((produto.precoPromocional || produto.preco) / 2).toFixed(2).replace(".", ",")} sem juros
                </p>
                <p className="text-sm text-green-600">ou R$ {(produto.precoPromocional || produto.preco).toFixed(2).replace(".", ",")} à vista</p>
              </div>

              <div className="mt-6 flex items-center gap-4">
                <div className="flex items-center gap-3">
                  <button onClick={() => setQuantidade(Math.max(1, quantidade - 1))} className="glass-card p-2 rounded-lg hover:bg-primary/10">
                    <Minus className="h-5 w-5" />
                  </button>
                  <span className="w-12 text-center font-semibold">{quantidade}</span>
                  <button onClick={() => setQuantidade(quantidade + 1)} className="glass-card p-2 rounded-lg hover:bg-primary/10">
              ...
              <div className="mt-6 flex gap-3">
                <button onClick={() => adicionarProduto({ ...produto, preco: produto.precoPromocional || produto.preco }, quantidade)} className="btn-primary flex-1 inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 font-semibold">
              ...
              <button onClick={handleWhatsApp} className="mt-3 w-full btn-glass inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 font-semibold">
              ...
              {relatedProducts.map((p) => (
                <Link key={p.id} to={`/produto/${p.id}`} className="glass-card group p-3">
                  <div className="aspect-square overflow-hidden rounded-xl bg-white/50">
                    <img src={p.imagem} alt={p.nome} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                  </div>
                  <h3 className="mt-3 text-sm font-medium line-clamp-2">{p.nome}</h3>
                  <p className="mt-1 font-bold text-primary">R$ {(p.precoPromocional || p.preco).toFixed(2).replace(".", ",")}</p>
                </Link>
              ))}
            </div>
          </div>
